...
// The httpClient parameter is optional; if nil, a default http.Client with a 30-second timeout is used.
func NewRestApiSession(accessToken string, httpClient *http.Client) *APIClient {
	client := httpClient
	if client == nil {
...
// It returns the key ID if successful, or an error if it fails.
...
func (s *APIClient) GetProjects(ctx context.Context) ([]ProjectInfo, error) {
	slog.Debug("Requesting projects")
	var response struct {
		Projects []ProjectInfo `json:"projects"`
	}
	err := s.request(ctx, "GET", "projects", nil, &response)
	if err != nil {
		return nil, err
	}
