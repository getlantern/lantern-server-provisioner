...
type listBillingAccountProjectsResponse struct {
	Projects []ProjectBillingInfo `json:"projectBillingInfo"`
}
...
// GetZoneRegionID returns a region ID like "us-central1".
func GetZoneRegionID(zoneID string) string {
	lastDashIndex := strings.LastIndex(zoneID, "-")
	if lastDashIndex == -1 {
		return zoneID
	}
	return zoneID[:lastDashIndex]
}

// GetID returns the cloud provider specific ID for the zone.
func (z Zone) GetID() string {
	return z.Name
}

// GetLocation implements the CloudLocation interface.
// It returns the physical location of the zone based on its region ID.
func (z Zone) GetLocation() *common.GeoLocation {
	...
	if item, ok := locationMap[GetZoneRegionID(z.Name)]; ok {
		return item
	}
	return nil
}
