	...
	session := &common.Session{
		...
		Cancel: func() {
			...
			default:
				cancel()
				select {
				case resultChan <- common.Event{Type: common.EventTypeOAuthCancelled}:
				default:
...
func startOauthFlow(ctx context.Context, openBrowser common.BrowserOpener, resultChan chan<- common.Event) {
	listener, listenErr := net.Listen("tcp", "127.0.0.1:0") // Listen on any available port
	if listenErr != nil {
		resultChan <- common.Event{Type: common.EventTypeOAuthError, Error: fmt.Errorf("failed to listen on port: %w", listenErr)}
		return
	}
	...
	mux.HandleFunc("GET /", func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Has("error") {
			if r.URL.Query().Get("error") == "access_denied" {
				...
				resultChan <- common.Event{Type: common.EventTypeOAuthCancelled}
			} else {
				http.Error(w, closeWindowHTML("Authentication failed"), http.StatusBadRequest)
				resultChan <- common.Event{Type: common.EventTypeOAuthError, Error: fmt.Errorf("oauth error: %s", r.URL.Query().Get("error"))}
			}
		...
		if err != nil {
			http.Error(w, closeWindowHTML("Authentication failed"), http.StatusBadRequest)
			resultChan <- common.Event{Type: common.EventTypeOAuthError, Error: fmt.Errorf("failed to exchange code: %w", err)}
			close(shutdownSignal) // Signal server shutdown
			return
		}
		w.Header().Set("Content-Type", "text/html")
		_, _ = fmt.Fprint(w, closeWindowHTML("Authentication successful"))
		resultChan <- common.Event{Type: common.EventTypeOAuthCompleted, Message: token.RefreshToken}
