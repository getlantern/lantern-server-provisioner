...
func NewAPIClient(ctx context.Context, refreshToken string) *APIClient {
	conf := &oauth2.Config{
...
func (c *APIClient) EnableServices(ctx context.Context, projectID string, data EnableServicesRequest) (*ServiceUsageOperation, error) {
...
type CreateProjectRequest struct {
	...
	Parent    *CreateProjectRequestParent `json:"parent,omitempty"`
}
type CreateProjectRequestParent struct {
	Type string `json:"type"`
	ID   string `json:"id"`
} // e.g., "organization", "folder"

// CreateProject creates a new GCP project. data should be a struct like:
func (c *APIClient) CreateProject(ctx context.Context, data CreateProjectRequest) (*ResourceManagerOperation, error) {
...
// ListBillingAccountProjects returns the projects linked to a billing account.
func (c *APIClient) ListBillingAccountProjects(ctx context.Context, accountName string) ([]string, error) {
	var resp listBillingAccountProjectsResponse
	err := c.doRequest(ctx, http.MethodGet, fmt.Sprintf("%s/%s/projects", billingV1API, accountName), nil, nil, &resp)
	if err != nil {
		return nil, err
	}
	var res []string
	for _, p := range resp.Projects {
		if p.BillingEnabled != nil && *p.BillingEnabled {
			res = append(res, p.ProjectID)
		}
	}
	return res, nil
}
...
func (c *APIClient) computeEngineOperationScopeWait(ctx context.Context, scope, url, operationName string) (*ComputeEngineOperation, error) {
	startTime := time.Now()
	for {
		select {
		case <-ctx.Done():
			return nil, fmt.Errorf("context cancelled/timed out while waiting for %s operation %s: %w", scope, operationName, ctx.Err())
		default:
			if time.Since(startTime) > operationPollTimeout {
				return nil, fmt.Errorf("timeout waiting for %s operation %s", scope, operationName)
			}
			var op ComputeEngineOperation
			err := c.doRequest(ctx, http.MethodGet, url, nil, nil, &op)
			if err != nil {
				// Don't necessarily stop polling on transient HTTP errors, but log/handle
				// If it's a 404, the operation might be invalid.
				var httpErr *HttpError
				if errors.As(err, &httpErr) && httpErr.StatusCode == http.StatusNotFound {
					return nil, fmt.Errorf("%s operation %s not found: %w", scope, operationName, err)
				}
				// Maybe retry a few times before failing hard
				slog.Debug("Warning: error polling operation. Retrying...", "op", operationName, "scope", scope, "err", err)
			} else {
				if op.Status == "DONE" {
...
func (c *APIClient) ComputeEngineOperationZoneWait(ctx context.Context, zone Locator, operationName string) (*ComputeEngineOperation, error) {
	return c.computeEngineOperationScopeWait(ctx, "zone", fmt.Sprintf("%s/operations/%s", zoneURL(zone), operationName), operationName)
}
...
func (c *APIClient) ComputeEngineOperationRegionWait(ctx context.Context, region RegionLocator, operationName string) (*ComputeEngineOperation, error) {
	return c.computeEngineOperationScopeWait(ctx, "region", fmt.Sprintf("%s/operations/%s", regionURL(region), operationName), operationName)
}
...
func (c *APIClient) ComputeEngineOperationGlobalWait(ctx context.Context, projectID string, operationName string) (*ComputeEngineOperation, error) {
	return c.computeEngineOperationScopeWait(ctx, "global", fmt.Sprintf("%s/global/operations/%s", projectURL(projectID), operationName), operationName)
}
...
func IsProjectHealthy(ctx context.Context, client *APIClient, projectID string) bool {
	bi, err := client.GetProjectBillingInfo(ctx, projectID)
	if err != nil {
		slog.Error("Error getting project billing info", "projectID", projectID, "error", err)
		return false
	}
	if bi == nil || bi.BillingEnabled == nil || !*bi.BillingEnabled {
		slog.Error("Project billing not enabled", "projectID", projectID)
		return false
	}
	services, err := client.ListEnabledServices(ctx, projectID)
	if err != nil {
		slog.Error("Error listing enabled services", "projectID", projectID, "error", err)
		return false
	}
	for _, service := range requiredServices {
		if !slices.ContainsFunc(services, func(s Service) bool {
			...
			slog.Error("Required service not enabled", "projectID", projectID, "service", service)
			return false
		}
	}
	return true
...
func CreateFirewallIfNeeded(ctx context.Context, client *APIClient, projectID string) error {
	// Check if the firewall already exists
	firewalls, err := client.ListFirewalls(ctx, projectID, fmt.Sprintf("name=%s", firewallName))
	if err != nil {
	...
	op, err := client.CreateFirewall(ctx, projectID, firewall)
	if err != nil {
...
func CreateInstance(ctx context.Context, client *APIClient, projectID string, zoneID string, publicSSHKey string) (string, string, error) {
	...
	loc := Locator{ZoneID: zoneID, ProjectID: projectID}
